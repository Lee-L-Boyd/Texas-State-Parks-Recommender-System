...
echo "<br/><br/>";
set_time_limit(0);
	$Social=intval($_SESSION['Social']);
	$Water=intval($_SESSION['Water']);
	$Active=intval($_SESSION['Active']);
	$Nature=intval($_SESSION['Nature']);

topPark($Social, $Water, $Active, $Nature);


?>
<span class="Button" onclick="window.location.href='create_profile.php';">Check out your profile on a graph</span>
</body>
</html>
<?php
...
function topPark($S, $W, $A, $N){
	...
	echo "1.) <a class='topTen' href=\"".$topScores[0]['website']."\">".$Parks[$topScores[0]['Park_ID']]."</a><br>";// (ParkNo".$topScores[0]['Park_ID'].") and the score is: ".$topScores[0]['Score']."<br>";
	echo "2.) <a class='topTen' href=\"".$topScores[1]['website']."\">".$Parks[$topScores[1]['Park_ID']]."</a><br>";// (ParkNo".$topScores[1]['Park_ID'].") and the score is: ".$topScores[1]['Score']."<br>";
	echo "3.) <a class='topTen' href=\"".$topScores[2]['website']."\">".$Parks[$topScores[2]['Park_ID']]."</a><br>";// (ParkNo".$topScores[2]['Park_ID'].") and the score is: ".$topScores[2]['Score']."<br>";
	echo "4.) <a class='topTen' href=\"".$topScores[3]['website']."\">".$Parks[$topScores[3]['Park_ID']]."</a><br>";// (ParkNo".$topScores[3]['Park_ID'].") and the score is: ".$topScores[3]['Score']."<br>";
	echo "5.) <a class='topTen' href=\"".$topScores[4]['website']."\">".$Parks[$topScores[4]['Park_ID']]."</a><br>";// (ParkNo".$topScores[4]['Park_ID'].") and the score is: ".$topScores[4]['Score']."<br>";
	echo "6.) <a class='topTen' href=\"".$topScores[5]['website']."\">".$Parks[$topScores[5]['Park_ID']]."</a><br>";// (ParkNo".$topScores[5]['Park_ID'].") and the score is: ".$topScores[5]['Score']."<br>";
	echo "7.) <a class='topTen' href=\"".$topScores[6]['website']."\">".$Parks[$topScores[6]['Park_ID']]."</a><br>";// (ParkNo".$topScores[6]['Park_ID'].") and the score is: ".$topScores[6]['Score']."<br>";
	echo "8.) <a class='topTen' href=\"".$topScores[7]['website']."\">".$Parks[$topScores[7]['Park_ID']]."</a><br>";// (ParkNo".$topScores[7]['Park_ID'].") and the score is: ".$topScores[7]['Score']."<br>";
	echo "9.) <a class='topTen' href=\"".$topScores[8]['website']."\">".$Parks[$topScores[8]['Park_ID']]."</a><br>";// (ParkNo".$topScores[8]['Park_ID'].") and the score is: ".$topScores[8]['Score']."<br>";
	echo "10.) <a class='topTen' href=\"".$topScores[9]['website']."\">".$Parks[$topScores[9]['Park_ID']]."</a><br>";// (ParkNo".$topScores[9]['Park_ID'].") and the score is: ".$topScores[9]['Score']."<br>";
}
...
function normalizeSWAN2(){
	include "newDBObj.php";
	$query=$mysqli->prepare("select Social,Water,Active,Nature from Park_Dimensions");
	$query->execute();
	$query->bind_result($Social1,$Water1,$Active1,$Nature1);
	if ($query->fetch()){
			$query->close();
	}
	$length=SQRT(POW(Social,2)+POW(Water,2)+POW(Active,2)+POW(Nature,2));
	$query=$mysqli->prepare("update Park_Dimensions set Social2=7*(Social/SQRT(POW(Social,2)+POW(Water,2)+POW(Active,2)+POW(Nature,2)))");
	$query->execute();
	$query->close();
	$query=$mysqli->prepare("update Park_Dimensions set Water2=7*(Water/SQRT(POW(Social,2)+POW(Water,2)+POW(Active,2)+POW(Nature,2)))");
	$query->execute();
	$query->close();
	$query=$mysqli->prepare("update Park_Dimensions set Active2=7*(Active/SQRT(POW(Active,2)+POW(Water,2)+POW(Active,2)+POW(Nature,2)))");
	$query->execute();
	$query->close();
	$query=$mysqli->prepare("update Park_Dimensions set Nature2=7*(Nature/SQRT(POW(Active,2)+POW(Water,2)+POW(Active,2)+POW(Nature,2)))");
	$query->execute();
	$query->close();
}
